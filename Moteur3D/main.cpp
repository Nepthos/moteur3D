...
void fillTriangle(VectInt p1, VectInt p2, VectInt p3, TGAImage &image, TGAColor color) {
    // VectInt bc = barycenter(p1,p2,p3);
...
void drawFacets(std::vector<VectInt> coordinates, std::vector<int> facets, TGAImage &image, std::vector<VectFloat> precise) {
    ...
    for (int i = 3 ; i < facets.size() - 3 ; i += 3) {
       //  std::cout << i << std::endl;
        ...
        //std::cout << "Norm vector before : x" << norm.x << ", y" << norm.y << ", z" << norm.z << std::endl;
        // TODO put normalize method inside object
        float length = std::sqrt(norm.x * norm.x + norm.y * norm.y + norm.z * norm.z);
        norm.x /= length;
        norm.y /= length;
        norm.z /= length;
        float intensity = dotProduct(norm, light);
        //std::cout << "Norm vector : x" << norm.x << ", y" << norm.y << ", z" << norm.z << std::endl;
...
void read_obj(TGAImage &image) {
    std::vector<VectInt> coordinates;
    std::vector<VectFloat> precise;
    ...
    for(int i = 0 ; i < coordinates.size() ; i ++) {
        VectInt p = coordinates.at(i);
        std::cout <<  "P: x:" << p.x << ", y:" << p.y << std::endl;

    }

    drawFacets(coordinates, facets, image, precise);
